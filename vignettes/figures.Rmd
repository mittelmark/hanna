...
```{r fig.width=14,fig.height=18,out.width=900}
par(mfrow=c(5,4),mai=c(0.0,0.5,0.5,0.0))
colors=c('grey80','#ff9999','#bb5555')
n=12
for (mode in c('null','memory1','memory5','gain','keystone')) {
    for (i in 1:30) {
        if (i == 1) {
            if (mode == "keystone") {
...
set.seed(128) 
par(mfcol=c(5,4),omi=c(0.5,0.7,0.5,0.3),mai=rep(0.2,4))  
for (i in c(1,5,10,30)) {
    res.df=simul_compare(n=10,seasons=i) 
    for (mod in c("null","memory1", "memory5","gain","keystone")) {  
       ...
       if (i == 1) {
          if (mod == "gain") {
            mtext("winlo",side=2,cex=1.5,line=3)
          } else {
...
```{r}
agg1=with(res.all,aggregate(pls, by=list(model,iter), mean))
agg2=with(res.all,aggregate(pls, by=list(model,iter), sd))
agg3=with(res.all,aggregate(wls, by=list(model,iter), mean))
agg4=with(res.all,aggregate(wls, by=list(model,iter), sd))
head(agg1,n=20)
head(agg2,n=20)
