...
for (mode in c('null','memory5','gain','keystone')) {
    for (i in 1:30) {
        ...
        D = simul$graph(res$M,mode='win') 
        cols=simul$colors(D,col=colors)
        if (i %in% c(1,5,10,30)) {
            simul$plot(D,vertex.color=cols,vertex.size=0.8)
            if (i == 1) {
                if (mode == "gain") {
...
set.seed(128) 
pdf("triads.pdf",width=14,height=14)
...
library(knitr)
knitr::kable(agg)
write.table(agg,file="path-lengths.tab",sep="\t",quote=FALSE)
```
